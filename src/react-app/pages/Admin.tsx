...
  Clock,
  User,
  FileText, // 💡 Ensure this icon is imported!
} from "lucide-react";
...
  // 💡 FIX START: Function to fetch FULL article data before setting state
  const handleEditArticle = async (article: Article) => {
    // 1. Set the basic article data immediately to open the modal quickly
...
        console.error("API Error fetching full article:", res.status, errorText);
        throw new Error("API failed to return OK status.");
      }
        
      const fullArticleData: Article = await res.json();
        
      // 3. Update the state with the full data. This triggers the form sync in the modal.
      setEditingArticle(fullArticleData);
    } catch (error) {
      console.error("Error loading full article details:", error);
      alert("Could not load full article details. Check console for API error.");
      setEditingArticle(null); // Close modal on failure
    }
  };
  // 💡 FIX END
...
          <h1 className="font-serif text-3xl font-bold text-gray-900">LuxeNest Admin</h1>
          <button onClick={handleLogout} className="flex items-center gap-3 text-red-600 hover:text-red-700 font-medium">
...
          {/* Add Buttons */}
          {(activeTab === "articles" || activeTab === "products" || activeTab === "rooms") && (
            <button
              onClick={() => {
...
              Create New {activeTab === "articles" ? "Article" : activeTab === "products" ? "Product" : "Room"}
            </button>
          )}

          {activeTab === "subscribers" && (
            <button onClick={handleExportCSV} className="mb-8 flex items-center gap-3 bg-gray-900 text-white px-8 py-4 rounded-xl font-bold">
...
                  <div className="text-5xl font-bold text-blue-900">{rooms.length}</div>
...
                  <div className="text-5xl font-bold text-green-900">{articles.length}</div>
...
                  <div className="text-5xl font-bold text-purple-900">{products.length}</div>
...
                            <title>Has long-form content</title>
                          </FileText>
                        )}
                      </td>
...
                              <span key={tag} className="text-xs bg-gray-200 text-gray-700 px-3 py-1 rounded-full">
                                {tag}
                              </span>
                            ))}
                          </div>
...
              {activeTab === "rooms" &&
                rooms.map((room) => (
                  <div key={room._id} className="flex justify-between items-center p-6 bg-gray-50 rounded-xl">
                    <div>
...
              {activeTab === "products" &&
                products.map((p) => (
                  <div key={p._id} className="flex justify-between items-center p-6 bg-gray-50 rounded-xl">
                    <div>
...
              <div key={s._id} className="flex justify-between items-center p-6 bg-gray-50 rounded-xl">
                <div>{s.email}</div>
